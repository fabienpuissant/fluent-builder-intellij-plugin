...
import io.fabienpuissant.fluent.builder.application.FluentBuilderApplicationService;
import io.fabienpuissant.fluent.builder.icons.FluentBuilderIcon;
...
public class DialogFieldArrangement extends DialogWrapper {
  ...
  protected JComponent createCenterPanel() {
    ...
    panel.add(
      createToolbar(fieldList)
        .addExtraAction(
          new AnAction(Message.MOVE_DOWN.label(), Message.MOVE_DOWN.label(), FluentBuilderIcon.MoveDown) {
            @Override
    ...
    panel.add(
      createToolbar(optionalFieldList)
        .addExtraAction(
          new AnAction(Message.MOVE_UP.label(), Message.MOVE_UP.label(), FluentBuilderIcon.MoveUp) {
            @Override
            public void actionPerformed(@NotNull AnActionEvent e) {
  ...
  private ToolbarDecorator createToolbar(JList<PsiField> fieldList) {
    return createDecorator(fieldList).disableAddAction().disableRemoveAction();
  }
  ...
  private void moveFields(JList<PsiField> sourceList, DefaultListModel<PsiField> sourceModel, DefaultListModel<PsiField> targetModel) {
    ...
    selectedFields.stream().filter(field -> !targetModel.contains(field)).forEach(targetModel::addElement);
  ...
  private List<PsiField> getSelectedFieldsFromList(JList<PsiField> list) {
    ...
    return IntStream.range(0, model.getSize()).mapToObj(model::getElementAt).collect(Collectors.toList());
  }
