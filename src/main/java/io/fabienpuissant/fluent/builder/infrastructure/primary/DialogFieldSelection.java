...
public class DialogFieldSelection extends DialogWrapper {
  ...
  private JBList<PsiField> loadClassFields(final PsiClass ownerClass) {
    CollectionListModel<PsiField> fields = new CollectionListModel<>(
      Arrays.stream(ownerClass.getFields()).filter(psiField -> !psiField.hasModifier(JvmModifier.STATIC)).toList()
    );
  ...
  private ToolbarDecorator createToolbar() {
    return createDecorator(fieldList).disableAddAction().disableRemoveAction().disableUpDownActions();
  }
